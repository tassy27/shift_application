...
1. `.env` を作成（`Copy-Item .env.example .env`）
2. `.env` の `DATABASE_URL` をローカルDB向けに設定
3. `npm install`
...
## CSV設定からの移行（互換運用）
- 旧 `config/runtime-params*.csv` を使って `.env` を生成したい場合のみ `npm run env:sync` を利用する。
- 現行の標準運用では `.env` を直接編集する。

## よくあるエラー
1. `DATABASE_URL is not set`
   - `.env` の `DATABASE_URL` を設定して再起動
2. `employee context is missing`
   - ログイン画面で社員選択 + メール入力ログインを実施
3. CSVが更新されない
   - 差分がない場合は新規CSVを作成しない仕様（互換運用時のみ該当）
